...
| T-050 | `scripts/check_docs.py` — referential integrity for `T-NNN`/`D-NNN` citations, frontmatter `last-updated`, dangling cross-links. Extend to detect stale hardcoded baseline numbers in prose (the `@verifier` residual risk on T-045: nothing mechanically prevents new ones) | 0 | P2 | — | 2026-08-28 |
| T-053 | `scripts/gate.py` has grown to ~490 lines and every other check depends on it, but it has no tests of its own. `parse_doctest` returns `{}` on unexpected output, which reads as "no metrics" rather than an error — a doctest format change would silently disable baseline checking | 0 | P1 | — | 2026-08-28 |
