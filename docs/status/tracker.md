...
| T-021 | Format debt: most tracked `src/`+`test/` files fail `clang-format` (current count in `scripts/baseline.json`). Decide bulk-reformat vs. leave, then widen the gate's default `--scope` | 0 | P1 | — | 2026-08-28 |
...
| T-025 | Gate is advisory only — nothing blocks a commit that skipped it. Add a git `pre-commit` hook running `scripts/gate.py`. **Promoted:** D-010 makes `git commit*` an allow-pattern on the `integrator` agent, which also matches `--no-verify` and `--amend`; opencode's pattern precedence is unverified, so the hook is now the only reliable guard on that hole | 0 | P0 | — | 2026-08-28 |
| T-054 | Verify after an opencode restart that an agent-level `allow` overrides a global `deny` (D-010). If it does not, `/sync` cannot commit at all and the `integrator` grant needs rethinking. Also confirm `git push` stays denied for `integrator` | 0 | P0 | — | 2026-08-28 |
| T-055 | `/sync` refuses on `master`/`main`, but recent work has been committed directly to `master`. Either adopt feature branches for anything non-trivial, or decide `/sync` should permit `master` with a warning. Until resolved, `/sync` is unusable in a master-only workflow | 0 | P1 | — | 2026-08-28 |
...
| T-051 | `architecture`, `cpp-conventions` and `add-a-test` skills — encode *this project*, not just the harness. Claims verified against source (`itool.h`, `permissions.{h,cpp}`, `read_file_tool.cpp`, `.clang-format`, `CMakeLists.txt`); writing them surfaced T-052 and corrected a false "test.exe blocks without --test" claim in `quality-gate` | 0 | opencode | 2026-08-28 |
| T-052 | `src/main.cpp:16` had `const bool testing = true;` with `run_tests = testing \|\| wants_tests(argv)`, so the `\|\|` could never be false: the suite ran regardless of `--test` and the GUI was unreachable, contradicting the comment above it. Fixed by sigmundur setting `testing = false`; the flag now genuinely controls the mode. Verified: no-flag run opens a window and blocks (no doctest output, alive after 12s); `--test` runs 26 cases | 0 | sigmundur | 2026-08-28 |
| T-053 | `scripts/gate.py` had no tests of its own, and `compare_baseline` failed **open**: metrics the parser did not produce were skipped, so a changed doctest summary format yielded an empty metric set and a confident `BASELINE: MATCH` that had checked nothing. Now fails closed. `scripts/test_tooling.py` adds 28 stdlib unit tests, run as gate step 0; a failure there stops the run. Guard confirmed by mutation: disabling it fails exactly the 2 regression tests | 0 | opencode | 2026-08-28 |
| T-050 | `scripts/check_docs.py` — `T-NNN`/`D-NNN` referential integrity, frontmatter, relative links, and hardcoded measurements in prose, with an explicit `<!-- check-docs: allow-missing -->` suppression that warns when it goes stale. Found 3 real defects on first run: dangling `T-012` and `D-002` in `mvp-roadmap.md`, and a hardcoded count this session had just introduced into `tracker.md` | 0 | opencode | 2026-08-28 |
| T-046 | `/sync` — the third lifecycle stage. Verifies the gate itself, updates tracker/decisions, stages explicitly, commits, prints a PR body, never pushes. Runs as the new `integrator` agent (D-010) | 0 | opencode | 2026-08-28 |
| T-043 | `/run` forbade committing while SPEC criteria were evaluated against `origin/master...HEAD` — the commits could not exist, and a verifier's `git diff origin/master` substitute produced a false finding. Resolved by D-010: `/sync` is the only committer, and `spec-format` now forbids revision-range criteria | 0 | opencode | 2026-08-28 |
| T-040 | Acceptance criteria naming a command whose output cannot distinguish pass from fail. `spec-format` now requires every criterion to state what the verifying command prints **when it fails**, and `@spec-auditor` checks it | 0 | opencode | 2026-08-28 |
